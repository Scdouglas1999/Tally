...
| Field | Value |
|---|---|
| App name (30) | `Tally for Jellyfin` |
...
| Privacy policy URL | `https://github.com/Scdouglas1999/Tally/blob/main/tally/PRIVACY.md` |
| Package | `io.github.scoduglas1999.jellytv` |

Full description:

```
Tally is a client for your own Jellyfin media server, made for the TV and the remote.
...
If your server runs the Tally plugin, a Tally section appears in the menu:
...
Without the Tally plugin it is a regular Jellyfin client.

Open source (GPL-2.0): github.com/Scdouglas1999/Tally
Not affiliated with the Jellyfin project.
```

Graphics (all in `tally/store/graphics/`): `icon-512.png`, `feature-1024x500.png`, `tv-banner-1280x720.png`,
...
- Release → Setup → **App signing**: choose *"Use a key you provide"* / "Export and upload a key from Java keystore"
  and upload from `~/.config/tally/release.jks` (alias `jellytv`). This keeps the Play build and the sideloaded
...
- Testing → **Internal testing** → create an email list with your friends' Google addresses (the ones their TVs
  use) → create release → upload `tally/out/Tally-play.aab` → roll out. Send them the opt-in link from the
...
Free developer account at developer.amazon.com. New app → Android → upload `tally/out/Tally-amazon.apk`,
