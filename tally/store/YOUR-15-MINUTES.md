...
2. Choose **Yourself** (personal account). Developer name: anything, e.g. `Tally`. It is public, as is your legal
   name once verified.
...
7. In the console: **Create app** → name `Tally for Jellyfin`, App, Free, tick the two declarations.
8. Everything after that is paste-and-upload from `LISTING.md`. Two ways to finish:
   ...
   - Setup → **API access** → create a service account, grant it "Release manager" on this app, download its
     JSON key to `~/.config/tally/play-api.json`. From then on `tally/release.sh --publish` can push every new
