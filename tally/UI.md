# Tally UI — the whole app in the Tally look

The Tally section (Games, Channels, Multiview), the player overlays and the plugin's web UI share one visual
language. As of 2026-09-22 the whole app uses it: the Wholphin look (pill buttons, round avatars, rounded posters,
full-page color washes) is retired for the TALLY theme. This document is the design authority for that work.
...
`tally/ui/theme/TallyTheme.kt` (`TallyColors`, `TallyType`, `TallyDimens`).
...
- Focus: 3dp accent border (`TallyDimens.focusBorder`), drawn inside the element's bounds; nothing moves.
...
- Upstream keeps the data: ViewModels, services, repositories, `BaseItem`, image URL helpers, navigation.
  Tally owns the composables. A Tally screen obtains the SAME ViewModel the upstream screen uses (same
  `hiltViewModel` call and factory arguments), reads the same state and calls the same actions. If a behavior
  lives in an upstream composable (a dialog, a context menu, a picker), the Tally screen reuses that composable
  rather than re-implementing it, until a later task restyles it.
- Routing: one seam at the top of `ui/nav/DestinationContent.kt` asks `TallyRoutes.Content(destination, …)` to
  render the destination; it returns false for anything Tally does not (yet) own, and the upstream `when` runs.
  Tally routes are active only while the TALLY theme is selected: choosing a Wholphin theme gives Wholphin's
  screens back, whole.
- Package layout: `tally/media/kit/` (components), `tally/media/<screen>/` (pages), strings in
  `res/values/strings_tally_media*.xml`.
- Every Tally page wraps its content in `TallyScale { … }` (1080p = 960x540dp) on `TallyColors.ground`.
- Theme-level coverage for everything not yet owned: square theme shapes (`TallyShapes`), no backdrop color wash
  (`TallyFirstRun`), IBM Plex type (`TallyTypography`).

## 3. The kit (`tally/media/kit/`)
Sizes are dp at the Tally canvas (960x540 at 1080p).
...
- played: a black tag top-left with mono 11sp `SEEN` in `muted` (Tally tag like multiview's MUTED).
...
**MediaRow** — `RowHeader` (mono uppercase + count, as on the Tally home row) above a `LazyRow` of cards with
`TallyDimens.cardGap`/2 = 9dp spacing, first card aligned to the page's left margin; remembers and restores the
focused index when focus re-enters the row.
**TallyButton** — square, height 40, horizontal padding 18, mono 14sp Medium UPPERCASE letter-spaced label with an
...
**Tabs** — the Tally top-bar tab style (indicator square + mono uppercase, selected = accent + underline),
generic over labels: `TallyTabs(labels, selected, onSelect)`.
...
- backdrop: the app-wide backdrop (upstream `BackdropService`, drawn once by `ApplicationContent`, image-only in
  the Tally look: top-right, fading out to the left and bottom). A page submits its item, exactly as upstream's
...
- left column (max 480): kicker (mono `label`, `muted`: `FILM`, `SERIES · 2 SEASONS`, `S1 · E4`), logo (max
  ...
  `CC · EN ES`;
- action row (16dp above the next section): TallyButtons, primary first.
