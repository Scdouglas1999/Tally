 ...
 * player ([LocalLiveTopBarActions]). The shared phone controls draw them with [LiveTopBarButtons]; until they do (they
 * are another screen's), the live player draws the same buttons itself at the top bar's end while the controls show.
 * Once the controls have drawn them, [drawnByControls] stays true and the live player's own copy is not drawn.
 */
class LiveTopBarActions {
    var actions by mutableStateOf<List<LiveTopBarAction>>(emptyList())

    var drawnByControls by mutableStateOf(false)
        internal set
}
