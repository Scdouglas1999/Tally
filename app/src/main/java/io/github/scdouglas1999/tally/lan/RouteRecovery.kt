 ...
 * again at the player's position, through the address now in use (the in-player Quality restart, which keeps the
 * chosen quality). Once per switch, and only for a network or HTTP error within a few minutes of it: anything else is
 * upstream's to handle.
 */
object RouteRecovery {
    ...
    fun recover(
        ...
        usedFor = switchedAt
        // a live stream the server closed (404): its stop report must not close the one opened again
        val playing = TallyQuality.nowPlaying.value
        if (playing?.liveStreamId != null && httpStatus(error) == 404) StaleLiveStop.expect(playing.playSessionId)
