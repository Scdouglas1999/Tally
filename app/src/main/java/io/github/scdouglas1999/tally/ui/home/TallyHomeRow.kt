...
/**
 * The Tally row on Wholphin's home screen: live and upcoming games as cards, above the library rows.
 *
 * Renders nothing (no header, zero height) when the server has no Tally plugin or there is nothing
 * worth watching, so a plain Jellyfin server sees upstream's home screen unchanged. The title is drawn
 * like [com.github.damontecres.wholphin.ui.cards.ItemRow]'s title; only the cards sit inside [TallyScale]
 * so they stay the size they are in the Tally section.
 *
...
fun TallyHomeRow(modifier: Modifier = Modifier) {
    ...
    // When the cards first appear right after the page opened, they take the initial focus (see TallyHomeFocus).
    val firstCardFocus = remember { FocusRequester() }
    LaunchedEffect(firstCardFocus) { TallyHomeFocus.claim(firstCardFocus) }

    // Same vertical rhythm as a home [ItemRow]: 8.dp between title and the card row, 8.dp of card-row
    // padding, and the 8.dp the home page adds under every library row. Card insets are inside [TallyScale]
    ...
    Column(
        ...
        TallyHomeRowTitle(anyLive = state.anyLive, count = state.games.size)
        TallyScale {
            LazyRow(
                ...
                modifier =
                    Modifier
                        ...
                        .focusGroup()
                        .focusRestorer()
                        .onFocusChanged { if (!it.hasFocus) TallyHomeHeaderState.focusedGame.value = null },
            ) {
...
/**
 * The row title: Tally's mono row header in the Tally look, else Wholphin's home-row title ([ItemRowTitle]:
 * `titleLarge`, `onBackground`, 8.dp start) plus a muted count.
 */
@Composable
private fun TallyHomeRowTitle(
    anyLive: Boolean,
    count: Int,
    modifier: Modifier = Modifier,
) {
    val title = stringResource(if (anyLive) R.string.tally_home_row_live else R.string.tally_home_row_today)
    HomeRowTitle(title = title, count = count, start = 20.dp) {
        Row(
            ...
            Text(
                text = stringResource(R.string.tally_home_row_count, count),
