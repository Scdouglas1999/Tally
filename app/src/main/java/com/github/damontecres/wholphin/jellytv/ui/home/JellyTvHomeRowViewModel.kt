            ...
            viewModelScope.launchIO {
                ...
                val text =
                    if (nudge.startsInMinutes == null) {
                        appContext.getString(R.string.jtv_actions_just_started, nudge.away, nudge.home)
                    } else {
                        appContext.getString(
                            R.string.jtv_actions_starts_in,
 ...
 */
internal fun List<JtvGame>.withFollowedTeamsFirst(favoriteTeams: Set<String>): List<JtvGame> {
    if (favoriteTeams.isEmpty() || isEmpty()) return this
    val teams = favoriteTeams.map { it.uppercase() }.toSet()

    fun List<JtvGame>.pin() = sortedWith(compareByDescending<JtvGame> { it.isFollowed(teams) })
...
private fun JtvTeam.shortLabel(): String = shortName.ifBlank { abbr.ifBlank { name } }
