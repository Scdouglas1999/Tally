...
/** The selected tab of the JellyTV page, so the top row of any tab's content can send UP straight to it. */
val LocalJtvUpTarget = compositionLocalOf<FocusRequester?> { null }
...
fun Modifier.upToTab(): Modifier =
    composed {
        val target = LocalJtvUpTarget.current
