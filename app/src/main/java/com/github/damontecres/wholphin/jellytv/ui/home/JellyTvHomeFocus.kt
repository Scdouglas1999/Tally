...
/**
 * Lets the JellyTV row claim the home screen's initial focus. Upstream focuses the first library row and scrolls
    ...
    suspend fun claim(firstCard: FocusRequester): Boolean {
        ...
        repeat(CLAIM_TRIES) {
            if (firstCard.tryRequestFocus("jellytv-home")) {
                claimed = true
                Timber.d("JellyTV home row claimed the initial focus")
                return true
            }
            delay(FRAME_MS)
        }
        Timber.d("JellyTV home row could not take focus")
        return false
    }
