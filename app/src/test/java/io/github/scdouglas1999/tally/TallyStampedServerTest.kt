...
class TallyStampedServerTest {
    ...
    fun `the stamped address is found after the signature pair`() =
        assertEquals(
            ...
            TallyStampedServer.read(apk(signature, pair(0x4A54560A, "http://203.0.113.7:9000".toByteArray()))),
        )
    ...
    fun `an end record with a comment does not hide it`() =
        assertEquals(
            ...
            TallyStampedServer.read(apk(signature, pair(0x4A54560A, "https://tv.example.org".toByteArray()), comment = "ci build")),
        )

    @Test
    fun `an ordinary release has no stamp`() = assertNull(TallyStampedServer.read(apk(signature)))

    @Test
    fun `only a web address is accepted`() =
        assertNull(TallyStampedServer.read(apk(signature, pair(0x4A54560A, "file:///data/x".toByteArray()))))

    @Test
    fun `a file that is not an apk is ignored`() =
        assertNull(
            TallyStampedServer.read(
                folder.newFile().also {
